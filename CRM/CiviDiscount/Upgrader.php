...
class CRM_CiviDiscount_Upgrader extends CRM_CiviDiscount_Upgrader_Base {
  ...
  public function upgrade_2204() {
    if (CRM_Core_BAO_SchemaHandler::checkIfFieldExists('cividiscount_item', 'is_visible')) {
      $this->ctx->log->info('Skipped cividiscount update 2204.  Column is_visible already present on cividiscount_item table.');
    }
    else {
      $this->ctx->log->info('Applying cividiscount update 2204.  Adding is_visible to the cividiscount_item table.');
      CRM_Core_DAO::executeQuery('ALTER TABLE cividiscount_item ADD COLUMN is_visible tinyint default 1');
    }
    return TRUE;
  }
